...
func Test_getCredentialData(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"RetrieveFunctionCredentials": {
			reason: "Should successfully retrieve function credentialss",
			args: args{
				req: fnv1.RunFunctionRequest{
		...
		"FunctionCredentialNotFound": {
			reason: "Should return nil if the credential is not found",
			args: args{
				req: fnv1.RunFunctionRequest{
					Credentials: map[string]*fnv1.Credentials{},
				},
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			GetNewTemplateWithFunctionMaps(nil, map[string]*fnv1.Credentials(tc.args.req.Credentials))
			got := getCredentialData("foo-creds")
